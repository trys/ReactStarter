...
(function() {
    ...
    self.addEventListener('message', function(event) {
    	if (event.data.command == 'trimCaches') {
            ...
            trimCache(version + jsonCacheName, 50);
    	}
    });
    ...
    self.addEventListener('fetch', function (event) {
        ...
        if (request.headers.get('Accept').indexOf('application/json') !== -1 || request.headers.get('Accept').indexOf('text/json') !== -1) {
            if (request.mode != 'navigate') {
                request = new Request(request.url, {
                    method: 'GET',
                    headers: request.headers,
            ...
            event.respondWith(
                fetch(request)
                    .then(function (response) {
                        // NETWORK
                        // Stash a copy of this page in the JSON cache
                        var copy = response.clone();
                        var cacheName = version + jsonCacheName;
                        stashInCache(cacheName, request, copy);
                        return response;
                    })
                    .catch(function () {
